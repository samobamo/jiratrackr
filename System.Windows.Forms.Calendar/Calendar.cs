...
namespace System.Windows.Forms.Calendar
{
        ...
        : ScrollableControl
    {
        ...
        public void Empty()
        {
            _renderer.Calendar.Items.Clear();
            foreach (CalendarDay day in _days)
            {
                day.ContainedItems.Clear();
                foreach (CalendarTimeScaleUnit TSU in day.TimeUnits)
                {
                    TSU.PassingItems.Clear();
                }
            }
            _items.Clear();
        }
            ...
            if (hitted != null)
            {
                switch (State)
                {
                    ...
                    case CalendarState.DraggingItem:
                        ...
                        itemOnState.StartDate = hitted.Date.AddMinutes(draggingOffsetInMinutes);
                        //fix <<<
