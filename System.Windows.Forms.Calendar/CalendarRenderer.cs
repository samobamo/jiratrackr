...
namespace System.Windows.Forms.Calendar
{
    ...
    public class CalendarRenderer
    {
        ...
        /// <summary>
        /// Draws bold text using the information of the <see cref="CalendarRendererBoxEventArgs"/>
        /// </summary>
        /// <param name="e"></param>
        protected virtual void DrawBoldBoxText(CalendarRendererBoxEventArgs e)
        {            
            TextRenderer.DrawText(e.Graphics, e.Text, e.Font, e.Bounds, e.TextColor, e.Format);
        }
        /// <summary>
        ...
        public virtual void OnDrawItemContent(CalendarRendererItemBoundsEventArgs e)
        {
            ...
            for (int i = 0; i < rectangles.Count; i++)
            {
                ...
                itemHeader = e.Item.webNo;
                Font fnt = new Font(e.Calendar.Font, FontStyle.Bold);
                SolidBrush myBrush = new SolidBrush(e.Item.HeaderColor);

                //rItemHeader = new Rectangle(Point.Empty, TextRenderer.MeasureText(itemHeader, fnt));
                rItemHeader = new Rectangle(Point.Empty, e.Bounds.Size);
                rItemHeader.X = bounds.Left;// + ItemTextMargin.Left;
                rItemHeader.Y = bounds.Top;// + ItemTextMargin.Top;
                //rItemHeader.Height = bounds.Height - ItemTextMargin.Vertical;
                rItemHeader.Height = 15;
                e.Graphics.FillRectangle(myBrush, rItemHeader);                
                                                              
                OnDrawItemWebNo(new CalendarRendererBoxEventArgs(e, rItemHeader, itemHeader, headerFontColor, fnt));
                Rectangle r = Rectangle.FromLTRB(
                    bounds.Left + ItemTextMargin.Left + rStartTime.Width,
                    bounds.Top + ItemTextMargin.Top + 15,
                    bounds.Right - ItemTextMargin.Right - rEndTime.Width,
                    bounds.Bottom - ItemTextMargin.Bottom);
                
                CalendarRendererBoxEventArgs evt = new CalendarRendererBoxEventArgs(e, r, e.Item.Text, e.Item.webNo, TextFormatFlags.Left | TextFormatFlags.Top);
        ...
        /// <summary>
        /// Draws the Jira Web no of the item if applicable
        /// </summary>
        /// <param name="e">Event data</param>
        public virtual void OnDrawItemWebNo(CalendarRendererBoxEventArgs e)
        {
            DrawBoldBoxText(e);
        }
        /// <summary>
