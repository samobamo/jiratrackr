...
using Version = JiraBrowserWin.JiraModel.Version;
...
namespace JiraBrowserWin
{
    ...
    public class JiraClient
    {
        ...
        private static void ValidateResponse(IRestResponse response)
        {
            ...
            if (response.ResponseStatus != ResponseStatus.Completed || response.ErrorException != null || response.StatusCode == HttpStatusCode.BadRequest || response.StatusCode == HttpStatusCode.NotFound)
        ...
        /// <summary>
        /// Returns all favourite filters. 
        /// Throws a JiraApiException if the request was unable to execute.
        /// </summary>
        /// <returns>Details of all projects visible to user</returns>
        public List<Filters> GetFavouriteFilters()
        {
            var request = new RestRequest()
            {
                Resource = ResourceUrls.FavouriteFilters(),
                RequestFormat = DataFormat.Json,
                Method = Method.GET
            };

            return Execute<List<Filters>>(request, HttpStatusCode.OK);
        }
        ...
        /// </summary>
        /// <returns>Details of all projects visible to user</returns>
        public List<Filter1> GetFavouriteFilters1()
        {
            var request = new RestRequest()
            {
                Resource = ResourceUrls.FavouriteFilters(),
            ...
            return Execute<List<Filter1>>(request, HttpStatusCode.OK);
        }
        ...
        /// <summary>
        /// Creates a new worklog. Throws a JiraApiException if the request was 
        /// unable to execute.
        /// </summary>
        /// <returns>the new worklog</returns>
        public Worklog CreateWorklog(CreateWorklog newWorklog, string issueKey)
        {
            var request = new RestRequest()
            {
                Resource = ResourceUrls.Worklog(issueKey),
                RequestFormat = DataFormat.Json,
                Method = Method.POST
            };

            request.AddBody(newWorklog);

            return Execute<Worklog>(request, HttpStatusCode.Created);
        }

        public Worklog CreateWorklog1(Worklog newWorklog, string issueKey)
        {
            var request = new RestRequest()
            {
                Resource = ResourceUrls.Worklog(issueKey),
        ...
        public Worklog UpdateWorklog(Worklog newWorklog, string issueKey)
        {
            var request = new RestRequest()
            {
                Resource = ResourceUrls.UpdateWorklog(issueKey, newWorklog.id),
                RequestFormat = DataFormat.Json,
                Method = Method.PUT
            };
